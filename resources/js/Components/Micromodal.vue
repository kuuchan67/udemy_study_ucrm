...
<template>
                        ...
                        <table class="table-auto w-full text-left whitespace-no-wrap">
                            ...
                            <tr v-for="customer in customers.value.data" :key="customer.id">
                                <td class="border-b-2 border-gray-200 px-4 py-3">
                                    <button @click="setCustomer({id: customer.id, kana: customer.kana})" type="button" class="text-blue-600">{{ customer.id }}</button></td>
    ...
    <input type="text" name="search" v-model='search' class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
    <button type="button" @click="searchCustomers" data-micromodal-trigger="modal-1" class="flex mt-4 ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">検索する</button>
</template>
